...
export default function EditAlertPage() {
  ...
  useEffect(() => {
    if (session?.user && alertId) {
      fetchAlert();
    } else if (status !== "loading") {
      setIsLoading(false);
    }
  }, [session, status, alertId]);
  ...
  const fetchAlert = async () => {
    try {
      ...
      setFilters({
        query: alert.filters.query || "",
        propertyTypes: alert.filters.propertyTypes || [],
        minPrice: alert.filters.minPrice?.toString() || "",
        maxPrice: alert.filters.maxPrice?.toString() || "",
        minSurface: alert.filters.minSurface?.toString() || "",
        maxSurface: alert.filters.maxSurface?.toString() || "",
        minRooms: alert.filters.minRooms?.toString() || "",
        maxRooms: alert.filters.maxRooms?.toString() || "",
        departments: alert.filters.departments || [],
        cities: alert.filters.cities || [],
        minRenovationScore: alert.filters.minRenovationScore?.toString() || "",
      });
    } catch (error) {
        ...
        <div className="container mx-auto max-w-2xl px-4 py-8">
          ...
          <div className="mb-8">
            ...
            <p className="text-muted-foreground">
              Modifiez les critères de votre alerte
            </p>
          </div>
              ...
              <CardContent className="p-6">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="80">80+ (Excellent potentiel)</SelectItem>
                  </SelectContent>
                </Select>
              </CardContent>
            </Card>
